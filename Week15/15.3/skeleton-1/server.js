...
var sequelizeConnection = models.sequelizeConnection
...
// a) sync the Manager table
.then(function(){
	return models.User.sync({force:true})
})
// b) then, sync the Uniforms table
.then(function(){
	return models.Fandom.sync({force:true});
})
// c) sync our new UserFandom model to a table
.then(function(){
	return models.UserFandom.sync({force:true})
})
